...
int cmdStatusEx(__in LPCTSTR BaseName, __in LPCTSTR Machine, __in DWORD Flags, __in int argc, __in_ecount(argc) TCHAR* argv[])
/*++
...
Routine Description:
    ...
    STATUSEX <id> ...
    use EnumerateDevices to do hardwareID matching
    for each match, dump status, resources, classes,
    stack, ids and driver nodes to stdout for device(s)
...
--*/
{
    ...
    context.count = 0;
    context.control = FIND_DEVICE | FIND_STATUS | FIND_RESOURCES | FIND_CLASS | FIND_STACK | FIND_DRIVERNODES | FIND_HWIDS;
    failcode = EnumerateDevices(BaseName,Machine,0,argc,argv,FindCallback,&context);
...
DispatchEntry DispatchTable[] = {
    ...
    { TEXT("statusex"),     cmdStatusEx,    MSG_STATUSEX_SHORT,    MSG_STATUSEX_LONG },
